...
if (container) {
  ...
  const waveSpacing = 900 / numWaves; // Space waves evenly
  const waves = [];
  ...
  function generateWavePath(yOffset, amplitude, frequency, phase, angle) {
    ...
    for (let x = 0; x <= 1000; x += 20) {
      ...
      let y =
        yOffset +
        noiseWaveFunction(x, amplitude, frequency, phase) +
        angleOffset;
      path += `L ${x},${y} `;
    }
  ...
  function createWave(index) {
    ...
    let frequency = Math.random() * 2 + 0.1;
